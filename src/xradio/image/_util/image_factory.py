...
def detect_image_type(store):
    ...
    if isinstance(store, str):
        # Classify from the image's own name only, not the full path: directory
        # components can contain these substrings by accident (e.g. a "Dropbox"
        # parent dir contains "pb" and would misclassify any image as a primary
        # beam).

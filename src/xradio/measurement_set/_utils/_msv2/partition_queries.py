...
def create_partitions(in_file: str, partition_scheme: list) -> list[dict]:
    ...
    time_state_table = time.time() - start
    logger.debug(f"Time to load state table: {time_state_table:.2f} seconds")
    ...
    time_update_partition_scheme = time.time() - start
    ...
    create_partition_time = time.time() - start
    logger.debug(f"Time to create partition info: {create_partition_time:.2f} seconds")
    return partitions


def create_partitions_new(in_file: str, partition_scheme: list) -> list[dict]:
    """Create a list of dictionaries with the partition information.

from xradio.vis import read_processing_set, load_processing_set, convert_msv2_to_processing_set
...
def base_test(msv2_name,expected_sum_value):
    ...
    for ms_xds_name in ps.keys():
        sum = sum + np.nansum(np.abs(ps[ms_xds_name].VISIBILITY*ps[ms_xds_name].WEIGHT))
        sum_lazy = sum_lazy + np.nansum(np.abs(ps_lazy[ms_xds_name].VISIBILITY*ps_lazy[ms_xds_name].WEIGHT))
    ...
    assert sum==sum_lazy, "read_processing_set and load_processing_set VISIBILITY and WEIGHT values differ."
    assert sum==pytest.approx(expected_sum_value, rel=relative_tolerance), "VISIBILITY and WEIGHT values have changed."

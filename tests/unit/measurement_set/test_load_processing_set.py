...
from xradio.measurement_set.load_processing_set import (
...
@pytest.fixture
def test_data_path():
    """Returns path to test measurement set"""
    # Download MS
...
class TestLoadProcessingSet:
    ...
    def test_selective_loading(self, test_ps_path):
        ...
        assert len(ms_names) == len(expected_names), "Number of measurement sets doesn't match"
        for ms_name, expected_name in zip(sorted(ms_names), sorted(expected_names)):
            assert ms_name == expected_name, f"Expected {expected_name} but got {ms_name}"

        # Test loading with selection parameters
        sel_parms = {
            ms_name: {'time': slice(0, 10)}
        }

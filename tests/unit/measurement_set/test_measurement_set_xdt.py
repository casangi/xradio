...
def create_test_ms_datatree(ms_type="visibility"):
    ...
    real_part = np.random.random((len(time), len(baseline), len(frequency), len(polarization)))
    imag_part = np.random.random((len(time), len(baseline), len(frequency), len(polarization)))
    ...
    field_source_ds = xr.Dataset(
        ...
        coords={
            'field_id': [0, 1],
            'line_name': (['field_id'], ['line1', 'line2'])  # Define line_name as a coordinate
        }
    )
...
class TestMeasurementSetXdt:
    ...
    def test_sel_with_polarization(self):
        ...
        if hasattr(pol_value, '__len__'):
            ...
            # It's a scalar value
            assert pol_value.item() == 'XX' if hasattr(pol_value, 'item') else pol_value == 'XX'
        
        # Check that the data variables have been correctly filtered
        assert len(data_vars_with_pol) > 0, "No data variables found with polarization dimension"
